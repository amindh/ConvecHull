...
def create_points(num_points, minimum=0, maximum=50):
    """
    Generates a list of fixed length of unique [x,y] coodinates as lists, 
    where each [x,y] is chosen randomly from a
    given range.
    ...
    :raise: a ValueError exception if the number of points is too large
    for the given boundaries
    """
...
def scatter_plot(points, convex_hulls=None, all_points=[], rays=None, 
                 minimum=0, maximum=50, title="convex hull",
                 show=False, save=False, rep='./figs/', prefix='convexhull_'):
    """
    Creates a scatter plot, input is a list of [x,y] coordinates.
    The second input 'convex_hull' is a list of list of [x,y] coordinates 
    consisting of those points in 'points' which
    make up some convex hulls.
    If not None, the elements of this list will be used to draw outer 
    boundaries (convex hulls surrounding the data
    points).
    ...
    :param save: if True, the drawing is saved in 'rep' directory as 
                    a .png image (default is False)
    :param rep: directory to save .png images
    :param prefix: prefix for the name of the .png images (that is followed 
                by a time stamp from :func:`time.time()
    time.time()`)
    :return: nothing
    """
    ...
    if rays is not None:
        for ray in rays:
            for i in range(1, len(ray) + 1):
                ...
                c0 = ray[i - 1]
                c1 = ray[i]
                plt.plot((c0[0], c1[0]), (c0[1], c1[1]), 'darkgray', 
                         linestyle=':')
...
def angle(point1, point2, point3):
    """
    Returns the angle (in radians formed) by three points. The second point 
    is the root of the angle.
    ...
    return math.acos(
        (distance(point2, point1) + 
         distance(point2, point3) - distance(point1, point3)) / (
                2 * math.sqrt(distance(point2, point1)) *
                math.sqrt(distance(point2, point3))))
...
def polar_quicksort(points, anchor):
    ...
    ensure increasing distance from the 'anchor'
    point.
    ...
    for pt in points:
        ...
        if pt_ang < piv_ang:
            ...
            larger.append(pt)
    return polar_quicksort(smaller, anchor) + sorted(equal, 
                          key=lambda x: distance(x, anchor)) + polar_quicksort(
        larger, anchor)
